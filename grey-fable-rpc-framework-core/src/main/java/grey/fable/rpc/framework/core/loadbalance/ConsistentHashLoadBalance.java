...
public class ConsistentHashLoadBalance extends AbstractLoadBalance {
    ...
    static class ConsistentHashSelector {
        ...
        static long hash(byte[] digest, int idx) {
            /*
             * 提取从 idx * 4 开始的 4 个字节, 每个字节 & 255 转换为无符号的 0-255 的值, 以防止符号扩展.
             * 这 4 个字节通过左移操作 << 拼接成一个 32 位的值, 每个字节的位移结果通过 | 运算符进行组合, 将各个字节拼接起来形成一个完整的 32 位值.
             * 将结果 & 4294967295L(即 & 0xFFFFFFFF), 用于将最终结果限制在无符号 32 位范围内, 确保返回的结果是 32 位的无符号值.
             */
